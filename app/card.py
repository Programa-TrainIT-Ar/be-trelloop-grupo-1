...
def create_card():
    try:
        ...
        tags_list = data.get('tags', [])  # <--- Asegúrate de obtenerlo
        ...
        last_position = db.session.query(func.max(Card.position))\
            .filter_by(board_id=board_id, state=state).scalar()
        ...
        # ---- Notificación si hay responsable ----
        if responsable_id:
